...
const builtins = {
    all: new Set(node_module.builtinModules),
    alwaysPrefixed: new Set(node_module.builtinModules.filter(mod => nodePrefixRx.test(mod)))
};
...
function externals(options = {}) {
    const config = Object.assign(Object.create(null), defaults, options);
    let include, exclude;
    const isIncluded = (id) => include.some(rx => rx.test(id));
    const isExcluded = (id) => exclude.some(rx => rx.test(id));
    return {
        name: 'node-externals',
        async buildStart() {
            ...
            const packageFiles = Array.isArray(config.packagePath)
